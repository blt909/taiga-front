...
class CardController
    ...
    _setVisibility: () ->
        relatedTaskVisibility = false
        slideShowVisibility = false

        if @.item.getIn(['model', 'tasks']) && @.item.getIn(['model', 'tasks']).size && @.visible('related_tasks')
            relatedTaskVisibility = true

        if @.item.get('images') && @.item.get('images').size && @.visible('attachments')
            slideShowVisibility = true

        visibility = {
            related: relatedTaskVisibility,
            slides: slideShowVisibility
        }

        if!_.isUndefined(@.item.get('foldStatusChanged'))
            if relatedTaskVisibility && slideShowVisibility
                visibility.related = !@.item.get('foldStatusChanged')
                visibility.slides = !@.item.get('foldStatusChanged')
            else if slideShowVisibility
                visibility.related = @.item.get('foldStatusChanged')
                visibility.slides = @.item.get('foldStatusChanged')
